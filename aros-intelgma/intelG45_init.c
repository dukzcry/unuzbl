...
/*
    ...
    $Id: intelG45_init.c 38735 2011-05-18 10:20:34Z neil $
*/
...
{
	/* Exclude cards which have GATT BAR more precisely */
	
		/* i915G/i915GM */
	return (product_id < 0x2582 || product_id > 0x2800) &&
		/* G33/Q33/Q35 */
		(product_id < 0x29b2 || product_id > 0x29d2) &&
		/* Pineview G/Pineview GM */
		product_id < 0xa000;
}

static BOOL HasOldBARsScheme(UWORD product_id)
{
	return
		/* i830/i845 */
		product_id == 0x2562 ||
		...
		/* i810/i815 */
		product_id == 0x1132 ||
		(product_id > 0x7120 && product_id < 0x7125);
}
BOOL Is_i8xx = FALSE;
    ...
    AROS_UFHA(APTR,         message,    A1))
{
	...
	OOP_AttrID mmio_bar, mmio_bar_size, window_bar, window_bar_size;
    	OOP_Object *driver;
	struct TagItem attrs[] =
	{
	...
	if(Is_i8xx = HasOldBARsScheme(ProductID)) {
                mmio_bar = aHidd_PCIDevice_Base1;
                mmio_bar_size = aHidd_PCIDevice_Size1;
                window_bar = aHidd_PCIDevice_Base0;
                window_bar_size = aHidd_PCIDevice_Size0;
	} else {
                mmio_bar = aHidd_PCIDevice_Base0;
                mmio_bar_size = aHidd_PCIDevice_Size0;
                window_bar = aHidd_PCIDevice_Base2;
                window_bar_size = aHidd_PCIDevice_Size2;
	}

        if (HasQuadBARs(ProductID))
        {
		OOP_GetAttr(pciDevice, mmio_bar, &mmio_base);
		OOP_GetAttr(pciDevice, window_bar, &window_base);

		OOP_GetAttr(pciDevice, mmio_bar_size, &mmio_size);
		OOP_GetAttr(pciDevice, window_bar_size, &window_size);
...
//    	sd->HardwareStatusPage = (APTR)readl(sd->Card.MMIO + 0x2080);
	...
	// talk only if lvds probe failed
        if (!lvds && probe_monitor(sd, G45_GPIOA ))
	    lvds = FALSE;
