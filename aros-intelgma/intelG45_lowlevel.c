...
/*
    ...
    $Id: intelG45_lowlevel.c 40197 2011-07-18 02:02:47Z neil $
*/
...
struct pll_restrictions {
	uint32_t M1_min, M1_max, M2_min, M2_max, m_min, m_max,
		N_min, N_max, VCO_min, VCO_max, P1_min, P1_max,
		p_min, p_max, PixelClock_min, PixelClock_max,
		P2_slow, P2_fast;	  	
};
static const struct pll_restrictions pll_limits[] = {
	/* i8xx DVO */
	{ 18, 26, 6, 16, 96, 140,
	  3, 16, 930000, 1400000, 2, 33,
	  4, 128, 25000, 350000, 
	  4, 2 },
	/* i8xx LVDS */
	{ 18, 26, 6, 16, 96, 140,
	  3, 16, 930000, 1400000, 1, 6,
	  4, 128, 25000, 350000,
	  14, 7 },
	/* i9xx families and followups */
	{ 10, 20, 5, 9, 70, 120,
	  3, 8, 1400000, 2800000, 1, 8,
	  5, 80, 20000, 400000,
	  NULL, NULL },
};
...
static BOOL calc_pll_and_validate(GMA_PLL_t *pll, struct pll_restrictions *pll_lims, struct g45staticdata *sd)
{
	uint32_t	m, p, refclk;
	...
	if (pll->M1 < pll_lims->M1_min || pll->M1 > pll_lims->M1_max)
		return FALSE;

	if (pll->M2 < pll_lims->M2_min || pll->M2 > pll_lims->M2_max)
		return FALSE;

	m = 5 * ((uint32_t)pll->M1 + 2) + (pll->M2 + 2);

	if (m < pll_lims->m_min || m > pll_lims->m_max)
		return FALSE;

	if (pll->N < pll_lims->N_min || pll->N > pll_lims->N_max)
		return FALSE;

	refclk = Is_i8xx ? sd->pipe == PIPE_A ? 48000 : 
		(readl(sd->Card.MMIO + G45_DPLLB_CTRL) & G45_DPLL_REF_INPUT_MASK) ==  G45_DPLLB_REF_INPUT_SPREADSPECTRIN ?
			/* XXX: Recheck freq */
			66000 : 
			48000 : 96000;
	pll->VCO = (refclk * m) / (pll->N + 2);

	if (pll->VCO < pll_lims->VCO_min || pll->VCO > pll_lims->VCO_max)
		return FALSE;

	if (pll->P1 < pll_lims->P1_min || pll->P1 > pll_lims->P1_max)
		return FALSE;

	p = (uint32_t)pll->P1 * (uint32_t)pll->P2;

	/* p range: for (s)DVO/DAC mode */
	if (p < pll_lims->p_min || p > pll_lims->p_max)
		return FALSE;

	pll->PixelClock = pll->VCO / p;

	if (pll->PixelClock < pll_lims->PixelClock_min || pll->PixelClock > pll_lims->PixelClock_max)
		return FALSE;
        ...
        uint16_t vstart, uint16_t vend, uint16_t vtotal, uint32_t flags)
{
	...
	clock.PixelClock = 0;
	struct pll_restrictions *pll_lims = Is_i8xx ? sd->pipe == PIPE_A ? &pll_limits[0] : &pll_limits[1] : &pll_limits[2];
	 ...
	 */
	if(Is_i8xx)
		t.P2 = pixelclock < 165000 ? pll_lims->P2_slow : pll_lims->P2_fast;
	else if (pixelclock <= 270000)
		t.P2 = 10;
	else
		t.P2 = 5;

	for (t.M1 = pll_lims->M1_min; t.M1 <= pll_lims->M1_max; t.M1++)
	{
		for (t.M2 = pll_lims->M2_min; t.M2 <= pll_lims->M2_max; t.M2++)
		{
			for (t.N = pll_lims->N_min; t.N <= pll_lims->N_max; t.N++)
			{
				for (t.P1 = pll_lims->P1_min; t.P1 <= pll_lims->P1_max; t.P1++)
				{
					if (calc_pll_and_validate(&t, pll_lims, sd) == TRUE)
					{
	...
	if (clock.PixelClock)
	{
		...
		if(clock.P1 == 2)
			...
			state->dpll |= (clock.P1 - 2) << G45_DPLL_P1_SHIFT;
		if (clock.P2 >= 4)
			state->dpll |= (1 << 23); /* DIVIDE BY 4 */
		
		//state->dpll |= (1 << (clock.P1 - 1)) << G45_DPLL_P1_SHIFT; /* IF LVDS! */
		...
		state->dspstride = Is_i8xx ? 
			(state->dspstride + 15) & ~15 :
			(state->dspstride + 63) & ~63;
