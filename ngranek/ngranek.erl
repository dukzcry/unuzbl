...
gen_faces(#vec_pair{left=Beg,right=Begv},#vec_pair{left=End,right=Endv},Color,#vec_pair{left=Pv,right=Ppv},#vec_pair{left=PovBeg,right=PovEnd},{Height,Hheight},SideDot1) ->
    %%Begx = get_intersect(Pv,Beg,Ppv,PovEnd),
    %%Endx = get_intersect(Pv,End,Ppv,PovEnd),
    %%BegYf = get_yf(Pv,Beg,Ppv,PovEnd),
    %%EndYf = get_yf(Pv,End,Ppv,PovEnd),
    %% projections of normals from dots to ppv ppv, no scaling
    ...
    %% projections of line between pov begin and dots on ppv, with scaling but with skew
    BegDot = #vec{x=Begx,y=BegYf(Begx)},
    EndDot = #vec{x=Endx,y=EndYf(Endx)},
    %% horizontal positions on screen
    PpBeg = vec_len(vec_op('-',BegDot,SideDot1)),
    PpEnd = vec_len(vec_op('-',EndDot,SideDot1)),
    Vpnf = get_vpnf(Pv,Ppv,PovEnd),
    %% normals from dots to ppv
    BegVpn = Vpnf(Beg),
    EndVpn = Vpnf(End),
