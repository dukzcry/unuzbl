...
Config_init() {
	...
	Config_normBgColor		:= "181818;303030;484848;606060;787878;909090;a8a8a8;c0c0c0;d8d8d8;f0f0f0"	; Normal background colour of bar elements; format: RRGGBB. The ten values are needed for indicating the number of windows per view (by default: darker colour means more windows) and distinct status information.
	...
	Config_selBgColor		:= "66667f;7f4444;7f0000"		; Background colour of 'selected' or highlighted bar elements (format: RRGGBB). The first colour is used for the layout and command symbol, the second for selected tags and dereasing battery level and the third color is only used for alarming battery level.
	Config_selFgColor		:= "c0c0c0;c0c0c0;ffffff"		; Foreground colour of 'selected' or highlighted bar elements.
	...
	Config_showTitleBars	:= False						; If false, the window title bars are hidden; they can be made visible or hidden by hotkey (see "decor" below).
	...
	; window arrangement
	Config_viewCount			:= 5						; The total number of views. This has effects on the displayed groups in the bar, and should not be exceeded in the hotkeys below.
	...
	Config_shellMsgDelay		:= 250						; The time bug.n waits after a shell message (a window is opened, closed or the focus has been changed); if there are any problems recognizing, when windows are opened or closed, try to increase this number.
	Config_viewFollowsTagged	:= False					; If true, the view is set to, if a window is tagged with a single tag.
	
	; Config_rules[i]	:= "<class (regular expression string)>;<title (regular expression string)>;<is managed (1 = True or 0 = False)>;<monitor (0 <= integer <= total number of monitors, 0 means the currently active monitor)>;<tags (binary mask as integer >= 0, e. g. 17 for 1 and 5, 0 means the currently active tag)>;<is floating (1 = True or 0 = False)>;<hide title (1 = True or 0 = False)>" (";" is not allowed as a character)
	Config_rules[1]		:= ".*;.*;1;0;0;0;0"				; At first you may set a default rule (.*;.*) for a default monitor and / or view.
	Config_rules[2]		:= "_sp;_sp;0;0;0;1;1"
	Config_rules[3]		:= "MozillaDialogClass;.*;0;0;0;0;1;0"
	Config_rules[4]		:= "gdkWindowToplevel;GIMP-Start;0;0;0;0;1;0"
	Config_rules[5]		:= "gdkWindowToplevel;GNU Image Manipulation Program;1;0;0;0;1;0"
	Config_rules[6]		:= "gdkWindowToplevel;Werkzeugkasten;1;0;0;0;1;0"
	Config_rules[7]		:= "gdkWindowToplevel;Ebenen, .* - Pinsel, Muster, .*;1;0;0;0;1;0"
	Config_rules[8]		:= "gdkWindowToplevel;Toolbox;1;0;0;0;1;0"
	Config_rules[9]		:= "gdkWindowToplevel;Layers, Channels, Paths, .*;1;0;0;0;1;0"
	Config_rulesCount	:= 9								; This variable has to be set to the total number of active rules above.
...
#F3::Run, C:\Program Files\Vim\vim72\gvim.exe -p Main.ahk Monitor.ahk Bar.ahk Config.ahk Manager.ahk View.ahk Session.ahk, D:\projects\bugn_800\source
#F4::Run, %COMSPEC% /c lynx.bat, C:\Program Files\lynx
#F5::Run, explorer D:\projects\bugn_800\source
#^End::Run, shutdown /p
...
#d::Manager_closeWindow()				; Close the active window.
#+d::Manager_toggleDecor()				; Show / Hide the title bar of the active window (only if Config_showTitleBars are set above).
...
#Tab::View_setLayout(-1)				; Set the previously set layout.
...
#BackSpace::Monitor_activateView(-1)	; Activate the previously activated view.
