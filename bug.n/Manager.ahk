...
Manager_applyRules(wndId, ByRef isManaged, ByRef m, ByRef tags, ByRef isFloating, ByRef hideTitle) {
	...
	If wndClass And wndTitle And Not (wndStyle & 0x80000000) 
	   And Not (wndX < -4999) And Not (wndY < -4999) {
		Loop, % Config_rulesCount {
			StringSplit, rule, Config_rules[%A_index%], `;
			If RegExMatch(wndClass . ";" . wndTitle, rule1 . ";" . rule2) {	; The last matching rule is returned.
				isManaged  := rule3
				m          := rule4
				tags       := rule5
				isFloating := rule6
...
Manager_cleanup() {
	Local sizeOfNONCLIENTMETRICS, NONCLIENTMETRICS, wndIds
...
Manager_getWindowInfo() {
	...
	If Manager[%aWndId%]_isFloating
		text .= " (floating)"
	MsgBox, , bugu.n: Window Information, % text
}
...
Manager_manage(wndId) {
	Local a, c0, hideTitle, isFloating, isManaged, m, tags, wndControlList0, wndX, wndY, wndWidth, wndHeight, wndProcessName
	
	If Not InStr(Manager_allWndIds, wndId ";")
		Manager_allWndIds .= wndId ";"
	Manager_applyRules(wndId, isManaged, m, tags, isFloating, hideTitle)
	...
	If isManaged {
		...
		Manager[%wndId%]_isDecorated := Config_showTitleBars
		...
		If (Manager[%wndId%]_tags & 1 << Monitor[%m%]_aView[1] - 1) {
			Manager_aMonitor := m
			Manager_winActivate(wndId)
			a := True
		} Else {
...
Manager_onShellMessage(wParam, lParam) {
	Local a, aWndHeight, aWndId, aWndWidth, aWndX, aWndY, flag, m, wndClass
